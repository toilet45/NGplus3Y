 ...
 * @member {MasteryStudyState[]} selectedStudies   Array of all given valid studies, whether or not they are actually
 *  accessible or purchasable in the given order
 * @member {MasteryStudyState[]} purchasedStudies  Array of studies which were actually purchased, using the given amount
...
export class MasteryStudyTree {
  ...
  constructor(studies) {
    ...
    switch (typeof studies) {
      case "string":
        ...
        if (MasteryStudyTree.isValidImportString(studies)) {
      ...
      case "undefined":
        ...
        break;
      default:
        throw new Error("Unrecognized input parameter for MasteryStudyTree constructor");
    }
  }
  ...
  static get currentStudies() {
    const currentStudies = player.mastery.studies.map(s => TimeStudy(s));
    if (player.challenge.eternity.unlocked !== 0) {
      currentStudies.push(MasteryStudy.eternityChallenge(player.challenge.eternity.unlocked));
    }
  ...
  parseStudyImport(input) {
    const studyDB = GameDatabase.eternity.timeStudies.mastery.map(s => s.id);
    const output = [];
    const studiesString = MasteryStudyTree.truncateInput(input).split("|")[0];
      ...
      for (const studyRange of studyCluster) {
        ...
        for (const study of studyArray) {
          if (studyDB.includes(parseInt(study, 10))) {
            const tsObject = MasteryStudy(study);
  ...
  attemptBuyArray(studyArray, checkCosts) {
    for (const study of studyArray) {
      const toBuy = typeof study === "object" ? study : MasteryStudy(study);
  ...
  hasRequirements(study, checkOnlyStructure = false) {
    ...
    const check = req => (typeof req === "number"
      ? this.purchasedStudies.includes(MasteryStudy(req))
      : req());
  ...
  get exportString() {
    return `${this.purchasedStudies
      .filter(s => s instanceof NormalMasteryStudyState)
      .map(s => s.id)
